...
import { minusCart, plusCart, removeCart } from '../../redux/actions/cartActionCreators';
        ...
        if(list[key].isbn13 !== undefined){
          ...
          items.push(
            <ListItem1
              ...
              onMinus={() => onMinus(list[key].isbn13)}
              onPlus={() => onPlus(list[key].isbn13)}
            />
          );
        }}
      }
...
const ListItem1 = ({ title, image, subtitle, onRemove, id, price, count, onMinus, onPlus} : IListItemProps & {onRemove:(id:number) => void} & {onPlus:(id:number) => void} & {onMinus:(id:number) => void}) => 
  <li className='cart-list-items' key={title}>
    ...
    <div className="cart-wrap-info">
      ...
      <Button className='cart-count-plus' onClick={() => onPlus(id) }>+</Button>
      <div className="cart-count">{count}</div>
      <Button className='cart-count-plus' onClick={() => onMinus(id) }>-</Button>
